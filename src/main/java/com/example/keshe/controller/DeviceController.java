...
public class DeviceController {
    ...
    public ResponseEntity<?> createDevice(@RequestBody Device device, HttpSession session) {
        ...
        try {
            ...
            if (device.getRoomLocation() == null || device.getRoomLocation().isEmpty()) {
                return ResponseEntity.badRequest().body(Map.of("error", "房间位置不能为空"));
            }
    ...
    public ResponseEntity<?> importTuyaDevices(HttpSession session) {
        ...
        try {
            Long familyId = (Long) session.getAttribute("familyId");
            if (familyId == null) {
                return ResponseEntity.badRequest().body(Map.of("success", false, "error", "请先创建或加入一个家庭"));
            }
            Map<String, Object> result = tuyaImportService.importUserDevices(familyId);
            return ResponseEntity.ok(result);
        } catch (Exception e) {
